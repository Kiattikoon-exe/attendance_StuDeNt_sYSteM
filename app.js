...
const staticPath = path.join(url.fileURLToPath(import.meta.url), "../views");
...
app.post("/saveAttendance", async (req, res) => {
  ...
  try {
    ...
    const { attendanceDate, attendanceTime, attendanceData } = req.body;

    // Process each attendance record
    for (const attendance of attendanceData) {
      // Get section_id from section number
      const sectionResult = await client.query(
        "SELECT id FROM section WHERE section = $1",
        [attendance.section]
      );

      if (sectionResult.rows.length === 0) {
        throw new Error(`Section ${attendance.section} not found`);
      }
      ...
      const studentCheck = await client.query(
        "SELECT id FROM student_list WHERE student_id = $1 AND section_id = $2",
        [attendance.studentId, sectionId]
      );

      if (studentCheck.rows.length === 0) {
        throw new Error(
          `Student ${attendance.studentId} not found in section ${attendance.section}`
        );
      }
      ...
      await client.query(
        `INSERT INTO student_list 
              (section_id, student_id, status, datecheck, timecheck)
              VALUES ($1, $2, $3, $4, $5)`,
        [
          sectionId,
...
app.get("/selectSection", (req, res) => {
  const { type, term } = req.query;

  if (type === "section") {
    // Query the database to find students in the specified section
    const query = "SELECT * FROM student WHERE section_id = $1";
    const values = [term]; // Assuming term is the section ID
...
app.get("/getStudentDetails/:studentId", async (req, res) => {
  ...
  try {
    ...
    const curriculumsQuery =
      "SELECT id, curr_name_en FROM curriculum ORDER BY id ASC";
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching student details" });
  }
});
...
app.post("/addStudentsToSection", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      success: false,
      message:
        error.message || "An error occurred while adding students to section",
    });
  } finally {
    client.release();
  }
...
app.post("/saveAttendance", async (req, res) => {
  const { sectionId, attendanceDate, attendanceTime, attendanceData } =
    req.body;
  const client = await pool.connect();
...
app.post("/addPrefix", async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query("SELECT * FROM prefix WHERE prefix = $1", [
      prefix,
    ]);
    ...
    if (result.rows.length > 0) {
      console.log(
        result.rows,
        "<<<< Prefix already exists. Please enter a new one."
      );
...
app.post("/addCurriculum", async (req, res) => {
  ...
  await pool.query(
    "INSERT INTO curriculum (curr_name_en, short_name_en) VALUES ($1, $2)",
    [curr_name_en, short_name_en]
  );
  res.redirect("/addStd");
});
...
app.post("/addStudent", async (req, res) => {
  ...
  const prefixResult = await pool.query(
    "INSERT INTO prefix (prefix) VALUES ($1) RETURNING id",
    [prefixId]
  );
...
app.post("/deleteStudent", async (req, res) => {
  ...
  try {
    const prefixResult = await pool.query(
      "SELECT prefix_id FROM student WHERE id = $1",
      [id]
    );
    if (prefixResult.rows.length > 0) {
      ...
      await pool.query("DELETE FROM prefix WHERE id = $1", [prefixId]);
